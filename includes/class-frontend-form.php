...
class WeForms_Frontend_Form {
    ...
    function submit_button( $form_id, $form_settings ) {
        if( isset($_POST['author_email'])){
            ...
            $messageVal = "Hi, " . $authorName . "!\n";
            $messageVal .= "You have received email from customer.\n";
        ...
        if( isset($author)){
            ...
            // wp_mail("wangstar1031@hotmail.com", "submit_button", "submit_button");
        ?>
        <input type="hidden" name="author_email" value="<?php echo $userData->user_email; ?>">
        <input type="hidden" name="author_name" value="<?php echo $userData->user_login; ?>">
        <?php
        }
        ?>
